...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  // Create safe CSS by sanitizing input values
  const sanitizeCSS = (value: string): string => {
    // Remove potentially dangerous characters and keep only valid CSS values
    ...
    .map(([theme, prefix]) => {
      const rules = colorConfig
        .map(([key, itemConfig]) => {
          ...
          if (!/^(#[0-9a-fA-F]{3,8}|hsl\([^)]+\)|rgb\([^)]+\)|[a-zA-Z]+)$/.test(sanitizedColor)) {
